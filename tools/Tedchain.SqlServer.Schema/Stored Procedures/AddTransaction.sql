CREATE PROCEDURE [Tedchain].[AddTransaction]
    @instance INT,
    @transactionHash BINARY(32),
    @mutationHash BINARY(32),
    @rawData VARBINARY(MAX),
    @records [Tedchain].[RecordMutationTable] READONLY
AS
    SET XACT_ABORT ON;
    SET NOCOUNT ON;
    ...
    LEFT OUTER JOIN [Tedchain].[Records] ON (UpdatedRecords.[Key] = Records.[Key] AND Records.[Instance] = @instance)
    WHERE UpdatedRecords.[Version] <> ISNULL(Records.[Version], CAST(0x AS VARBINARY(32)));

    IF @@Rowcount > 0
        RETURN;

    MERGE [Tedchain].[Records] AS Target
    ...
    INSERT INTO [Tedchain].[Transactions]
    ...
    INSERT INTO [Tedchain].[RecordMutations]
